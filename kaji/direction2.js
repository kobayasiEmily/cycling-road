...
function plotElevation({ results }) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (i > 0) {
      // 標高差分を計算
      diffEleArray.push(results[i].elevation - results[i - 1].elevation)
    }
  }
  ...
  for (var i = 0; i < diffEleArray.length; i++) {
    if (diffEleArray[i] > 9) {
      datasets[0].backgroundColor[i+1] = '#FF5E5B'
    }else if(diffEleArray[i] < 0){
      datasets[0].backgroundColor[i+1] = '#74ff5b'
    }else{
      datasets[0].backgroundColor[i+1] = '#5b6eff'
    }
  }
